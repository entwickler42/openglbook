...
#define WINDOW_TITLE_PREFIX "Chapter 2.3"
...
const GLchar* VertexShader = { "#version 130\n"
...
const GLchar* FragmentShader = { "#version 130\n"
...
void timer_callback(int tid) {
	if (tid == 1) {
		char buf[512];
		snprintf(buf, 512, "%s: %i FPS @ %i x %i", WINDOW_TITLE_PREFIX,
				g_cfps * 4, g_width, g_height);
...
int main(int argc, char* argv[]) {
	...
	if (g_hwnd < 1)
		error_at_line(-1, 0, __FILE__, __LINE__ - 2,
	...
	GLenum rs = glewInit();
	if (rs != GLEW_OK)
		error_at_line(-1, 0, __FILE__, __LINE__ - 2, "%s",
				glewGetErrorString(rs));
